...
export const SEED_FILES: readonly { name: string; text: string }[] = [
  ...
  { name: "DESKTOP\\games\\CHESS.EXE", text: programBody("chess") },
  {
    ...
    text: [
      ...
      "",
      "The processor is real. See /docs/asm.txt. So is the",
      "drive, and there is a language model on it. In the",
      "terminal: cd /src, then cc llm.c, then run llm. It",
      "is slow because it is doing the arithmetic, not",
      "because it is waiting for anything.",
    ].join("\n"),
  },
  {
  ...
  },
  {
    ...
    text: [
      ...
      "THE SCREEN",
      "",
      "  There is also a screen: 40 cells across, 24 down,",
      "  one character each. It lights at the first",
      "  character written to it and takes the terminal's",
      "  place until the program ends.",
      "",
      "  VPOS  0x0F04  the cursor, a cell 0..959. Cell =",
      "                row * 40 + column. Writes wrap;",
      "                reading answers where it is",
      "  VCHR  0x0F05  write a character, it lands at the",
      "                cursor and the cursor moves on.",
      "                Reading answers what is there",
      "  VSYNC 0x0F06  read it and the processor rests",
      "                until the next frame of the display,",
      "                about a sixtieth of a second. The",
      "                value counts frames",
      "",
      "THE DRIVE",
      "",
      "  There is more on this machine than fits in it. The",
      "  drive holds the rest, and the processor reaches it",
      "  a byte at a time through three more ports.",
      "",
      "  DPOS  0x0F07  where the head is, low word",
      "  DBNK  0x0F08  the same address, high word. Together",
      "                they count bytes, not words, so the",
      "                drive can be larger than memory",
      "  DSK   0x0F09  read a byte and the head moves on;",
      "                write one and the same. Past the end",
      "                of the media a read is 0 and a write",
      "                goes nowhere",
      "",
      "  Both halves of the address read back, and the head",
      "  carries from one into the other, so a long run",
      "  costs one LD an instruction. Writes stay on the",
      "  drive until the program ends; the media comes back",
      "  whole for the next one.",
      "",
      "  What is on it is a language model: 260,032 numbers,",
      "  one byte each. /src/llm.c reads them.",
      "",
  ...
  },
  {
    ...
    text: [
      ...
      "  rand()    16 random bits",
      "  vpos(p)   aim the screen cursor at cell p.",
      "            The screen is 40 x 24; cell =",
      "            row * 40 + column. The first",
      "            character drawn turns it on",
      "  vput(c)   put character c at the cursor;",
      "            the cursor moves on",
      "  vsync()   rest until the display's next",
      "            frame; about sixty arrive a",
      "            second. Answers the frame count",
      "  dpos(a)   aim the drive head, low word",
      "  dbank(a)  the same address, high word",
      "  dget()    a byte off the drive; the head",
      "            moves on",
      "  dput(v)   a byte onto it, likewise",
      ...
      "  the assembler, for when C is not enough. A",
      "  $name in one of those lines is wherever the",
      "  compiler put that name: a global, another",
      "  function, or one of this function's own",
      "  arguments. That is how hand-written assembly",
      "  and the C around it agree about where things",
      "  are. R5 and R7 belong to the compiler; the",
      "  other six registers are yours.",
      "",
      "SMALL PRINT",
      "",
      "  Numbers are 16 bits and wrap. >> shifts in",
      "  zeros. #define takes a name and a number and",
      "  nothing else on the line — but sums of",
      "  constants are worked out while compiling, so",
      "  a size can be written as one.",
      "",
      "  A function that cannot reach itself, however",
      "  far round, keeps its arguments and locals at",
      "  fixed addresses rather than on a stack: it is",
      "  smaller and quicker that way, and it is what",
      "  lets $name in asm find them. Anything that",
      "  does recurse gets a frame as it always did.",
      "  The heap starts where the program ends and",
      "  grows up toward 0x0E00. Nobody referees. The",
  ...
  },
  {
    ...
    text: [
      ...
      "   cd /src; cc pong.c; run pong. W and S move you. First to seven.",
      "   The screen ports, wearing C: vpos(row * 40 + col) aims the",
      "   cursor, vput(c) prints there and moves on, vsync() rests until",
      "   the display's next frame. See /docs/c.txt. ESC leaves, always. */",
      "",
      "int py = 10;            /* your paddle's top row */",
      "int ay = 10;            /* its paddle's top row */",
      "int bx;  int by;        /* the ball */",
      "int dx;  int dy;",
      "int ps = 0;  int as = 0;  /* the score, you : it */",
      "int pause = 0;          /* frames of rest before a serve moves */",
      "",
      ...
      "}",
      "",
      "void bar(int x, int top, int c) {",
      "    int i;",
      "    for (i = 0; i < 4; i++) draw(x, top + i, c);",
      "}",
      "",
      "void court() {",
      "    int i;",
      "    for (i = 0; i < 40; i++) { draw(i, 0, '='); draw(i, 23, '='); }",
      "    text(16, 23, \" W AND S \");",
      "}",
      "",
      "void score() {",
      ...
      "}",
      "",
      "/* the loser serves: the ball leaves them and goes to the scorer */",
      "void serve(int toward) {",
      "    bx = 20;  by = 11;",
      "    dx = toward;",
      "    dy = (rand() & 1) ? 1 : -1;",
      "    pause = 45;",
      ...
      "    if (ny < 1) ny = 1;",
      "    if (ny > 19) ny = 19;",
      "    if (ny != py) { bar(2, py, ' '); py = ny; bar(2, py, '|'); }",
      "}",
      "",
      "/* it watches the ball and is not in a hurry */",
      "void machine() {",
      "    if (by > ay + 2 && ay < 19) { bar(37, ay, ' '); ay++; bar(37, ay, '|'); }",
      "    if (by < ay + 1 && ay > 1) { bar(37, ay, ' '); ay--; bar(37, ay, '|'); }",
      "}",
      "",
      "/* one step of the ball; walls and paddles have their say */",
      "void moveball() {",
      "    int ny;",
      "    draw(bx, by, ' ');",
      "    ny = by + dy;",
      ...
      "    by = ny;",
      "    bx = bx + dx;",
      "    if (bx == 2 && dx < 0 && by >= py && by <= py + 3) {",
      "        dx = 1;  bx = 3;",
      "        if (by == py) dy = -1;      /* the edges put english on it */",
      "        if (by == py + 3) dy = 1;",
      "    }",
      "    if (bx == 37 && dx > 0 && by >= ay && by <= ay + 3) {",
      "        dx = -1;  bx = 36;",
      "        if (by == ay) dy = -1;",
      "        if (by == ay + 3) dy = 1;",
      "    }",
      ...
      "        bt++;",
      "        if (bt == 3) { bt = 0; moveball(); }",
      "        mt++;",
      "        if (mt == 4) { mt = 0; machine(); }",
      "    }",
      "    if (ps > as) text(10, 11, \"THE MACHINE CONCEDES.\");",
      "    else text(6, 11, \"THE MACHINE WINS. OF COURSE.\");",
      "    text(16, 13, \"ANY KEY.\");",
      "    while (key()) ;",
      "    while (!key()) vsync();",
      "    return 0;",
